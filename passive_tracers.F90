...
module passive_tracers
  ...
  logical (kind=log_kind) ::  &
    ecosys_on, cfc_on, iage_on, moby_on, tr3he_on
...
contains
  ...
  subroutine init_passive_tracers(init_ts_file_fmt, &
    ...
    call broadcast_scalar(tr3he_on,  master_task)
    ...
    if (tr3he_on) then
      call set_tracer_indices('TR3HE', tr3he_tracer_cnt, cumulative_nt,  &
        tr3he_ind_begin, tr3he_ind_end)
    end if
    ...
    if (tr3he_on) then
      ...
      if (errorCode /= POP_Success) then
        call POP_ErrorSet(errorCode, &
          'init_passive_tracers: error in tr3he_init')
        return
      endif
  ...
  subroutine set_interior_passive_tracers_3D (TRACER_OLD, TRACER_CUR)
    ...
    !-----------------------------------------------------------------------
    !  TR3HE does not compute and store 3D source-sink terms
    !-----------------------------------------------------------------------
  ...
  subroutine write_restart_passive_tracers(restart_file, action)
    ...
    !-----------------------------------------------------------------------
    !  TR3HE does not write additional restart fields
    !-----------------------------------------------------------------------
  ...
  function tracer_ref_val(ind)
    ...
    !-----------------------------------------------------------------------
    !  TR3HE does not use virtual fluxes
    !-----------------------------------------------------------------------
